        ...
        <template v-for="(item, index) in renderCol" :key="index">
          ...
          >
            <template slot-scope="{ row }">
              ...
              <!-- 集群id -> 集群名 -->
              <template v-else-if="item.value === 'assetLbGroup'">
              ...
              <template v-else-if="item.value === 'instanceStatus'">
                <gs-tag :type="lvsStatusMap[row[item.value] || '未知']">{{ row[item.value] || '未知' }}</gs-tag>
              </template>
              <template v-else-if="item.value === 'rsUuidListMappingIps'">
...
export default {
  ...
  data() {
    return {
      storeNamespace: 'ASSETS_LVS_TABLE',
      form: {
        productList: [],
        idcList: [],
      },
  ...
  methods: {
    ...
    async del(instanceName) {
      const res = await deleteLvs(instanceName);
      if (res.error_code !== 0) {
